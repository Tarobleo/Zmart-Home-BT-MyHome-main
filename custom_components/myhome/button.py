...
class DisableCommandButtonEntity(ButtonEntity, MyHOMEEntity):
    ...
    async def async_press(self) -> None:
        """Press the button."""
        command = OWNCommand.parse(f"*14*0*{self._full_where}##")
        if command is None:
            LOGGER.error("Could not parse disable command for `%s`.", self._full_where)
            return
...
class EnableCommandButtonEntity(ButtonEntity, MyHOMEEntity):
    ...
    async def async_press(self) -> None:
        """Press the button."""
        command = OWNCommand.parse(f"*14*1*{self._full_where}##")
        if command is None:
            LOGGER.error("Could not parse enable command for `%s`.", self._full_where)
            return
        await self._gateway_handler.send(command)
